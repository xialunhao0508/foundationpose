...
class ScorePredictor:
  def __init__(self, ckpt_dir, amp=True):
    self.amp = amp
    ...
    a_path = os.path.dirname(f'{code_dir}')
    b_path = os.path.dirname(a_path)
    c_path = os.path.dirname(b_path)
    self.cfg = OmegaConf.load(f'{code_dir}/../../config/{self.run_name}/config.yml')
    # path_cfg = os.path.join(c_path, 'tests/weights', f'{self.run_name}/config.yml')
    # self.cfg = OmegaConf.load(path_cfg)

    # ckpt_dir = os.path.join(c_path, 'tests/weights', f'{self.run_name}/{model_name}')
    ...
    self.dataset = ScoreMultiPairH5Dataset(cfg=self.cfg, mode='tests', h5_file=None, max_num_key=1)
