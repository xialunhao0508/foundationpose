...
class SHREC16(Dataset):
    ...
    Args:
        ...
        split (str): String to indicate what split to load, among ["train", "val", "tests"].
                     Default: "train".
        transform (Callable):
    ...
    def __init__(self, root: str, categories: list = None, split: str = "train",
        ...
        if split == "tests":
            # Setting synsets and labels to None if in tests split
            self.synsets = [None]
            self.labels = [None]
        else:
        ...
        if split == "tests":
            ...
            for i in range(len(self.synsets)):
                ...
                if split == "train":
                    ...
                    raise ValueError(f'Split must be either train, tests or val, '
                                     f'got {split} instead.')
