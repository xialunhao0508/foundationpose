...
class TestTimelapse:
    def test_add_mesh_batch(self, out_dir, meshes, material_values, material_textures):
        ...
        for iteration, params in data.items():
            writer.add_mesh_batch(iteration=iteration, category='tests', **params)

        # Check that category directory is created
        assert os.path.exists(os.path.join(out_dir, 'tests'))

        # Check that data at each iteration is correct
        texture_dir = os.path.join(out_dir, 'tests', 'textures')
        assert os.path.exists(texture_dir) 
        for iteration in data.keys():
            filename = os.path.join(out_dir, 'tests', 'mesh_0.usd')
    ...
    def test_add_voxelgrid_batch(self, out_dir, voxelgrid):
        ...
        for iteration, params in data.items():
            writer.add_voxelgrid_batch(iteration=iteration, category='tests', **params)

        # Verify
        filename = os.path.join(out_dir, 'tests', 'voxelgrid_0.usd')
        for iteration, params in data.items():
    ...
    def test_add_pointcloud_batch(self, out_dir, pointcloud):
        ...
        for iteration, params in data.items():
            writer.add_pointcloud_batch(iteration=iteration, category='tests', **params)

        # Verify
        filename = os.path.join(out_dir, 'tests', 'pointcloud_0.usd')
        for iteration, params in data.items():
    ...
    def test_add_pointcloud_batch_instancer(self, instancer_out_dir, pointcloud):
        ...
        # Verify
        filename = os.path.join(instancer_out_dir, 'tests', 'pointcloud_0.usd')
        for iteration, params in data.items():
            pointcloud_in = io.usd.import_pointcloud(filename, scene_path='/pointcloud_0', time=iteration)[0]
...
class TestTimelapseParser:
    ...
    def timelapse_sample_dir(self):
        ...
        #    --test_objs=tests/samples/rocket.obj,tests/samples/model.obj --output_dir=<CLEARED_OUTPUT_DIR>
        cur_dir = os.path.dirname(os.path.realpath(__file__))
