...
class TestAttributes:
    ...
    def test_set_batching(self, device, batching, convert_to_batching, method):
        """ Convert to another batching strategy and tests that the output is as expected. """
        input_attr = make_default_input(device, batching)
        mesh = construct_mesh_default(input_attr)
    ...
    def test_check_sanity(self, device, batching):
        """ Check that unexpected tensor sizes are detected (valid arguments already tested in the construction tests).
        """
        input_attr = make_default_input(device, batching)
        mesh = construct_mesh_default(input_attr)
        assert mesh.batching == batching
    ...
    @pytest.mark.parametrize("method_to_test", ["to", "named"])  # if to tests mesh.to or mesh.cpu/mesh.cuda
    ...
    def test_device_convert(self, device, batching, to_device, method_to_test):
        ...
        if method_to_test == "to":
            ...
            raise RuntimeError(f'Bug; unknown tests condition.')
